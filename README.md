...
- `## 名字` 是專案分區
- 每行開頭的行內程式碼是狀態，只有這七個算數：待辦、提案中、暫存、實作中、待合併、已歸檔、完成
...
5. 前四條都對不上，但那一行**帶生命週期狀態**（待辦／提案中／暫存／實作中／待合併／已歸檔）
...
| 看板 | 六欄生命週期，卡片可拖曳改狀態；可只看某一天推進的項目 |
...
ui/                前端（靜態檔，沒有 build step）
  index.html       月曆首頁
  board.html       看板（拖曳改狀態）
  js/store.js      向後端要資料，所有頁面共用；頁面程式包在 Log.ready() 裡
  js/shell.js      頂端工具列、連結面板、未寫回徽章、資料夾異常橫幅
  css/app.css      全站隱藏捲軸
...
導覽是頂端一條工具列：左邊品牌與麵包屑，正中間是分段控制，右邊是未寫回徽章與深淺色。
