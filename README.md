...
設定與 TODO 存在 `~/Library/Application Support/tw.npust.worklog-app/`，不會寫進日誌資料夾。
（例外只有日誌規則：那一區直接改 `~/.claude/CLAUDE.md`，改前會備份。）
日誌資料夾裡會被寫到的只有「加到今日日誌」新增的那一行，以及看板改狀態動到的那一行。
...
| 看板 | 七欄生命週期，卡片可拖曳改狀態（直接寫進今天的 md）；可只看某一天推進的項目 |
...
看板上把卡片拖到別欄（或用卡片右上角的「移動到…」），app **當下就改今天的 `<民國7碼>.md`**，
沒有「待寫回」這種中間狀態，畫面看到的永遠就是檔案裡的內容。

寫法分兩種：

- **今天的檔案裡已經有這支工作項目的行** → 只把行首的狀態標籤換掉。
  標題、連結、括號補充、縮排、標籤後面的空白**一個字都不動**，其他行也不重排
- **今天還沒有這支項目的行** → 在對應的 `## 專案` 區塊最後一個 bullet 後面新增一行；
  沒有那個專案區塊就在檔尾開一個，檔案不存在就建一個

新增的那一行長這樣，跟你自己寫的格式一致，所以下次重讀還會歸到同一支工作項目：

```markdown
- `實作中` [search-message-history：對話紀錄搜尋](https://redmine.example.com/issues/32979)
```

- 標題寫成 `slug：描述`；`auto-` 開頭的項目 id 是雜湊算出來的（見〈工作項目是怎麼算出來的〉第 5 條），
  那種不加前綴，直接用原本的標題——它本來就是靠連結歸戶的，不加也認得出來
- 連結依狀態選：`待合併`／`已歸檔` 用 MR，其餘用議題；兩個都沒有就只寫標題
- 同一天連拖兩次不會長出兩行，第二次改的是第一次寫出來的那一行
- 定位靠的是**行號**（解析時就記著每個條目來自第幾行），不是拿整行去比對，
  所以同一天出現兩行長得一樣的條目也不會改錯

寫完會浮一則提示，例如「已寫進 1150820.md：測試中」，資料也立刻重讀，日誌頁與月曆同步跟上。
寫不進去的時候（還沒設資料夾、資料夾不存在、沒有權限…）會用紅字說明原因，
**而且看板會重畫一次，卡片退回原本的欄位**——畫面絕不會顯示一個檔案裡沒有的狀態。

拖到卡片原本就在的那一欄不算改變，什麼都不寫。
...
- 看板改狀態會動到你的日誌檔，而且沒有「復原」：改錯了要自己去 md 改回來
