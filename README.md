...
讀資料夾裡的 `<年>/<月>/<西元8碼>.md`，把每天做了什麼、每支工作項目走到哪一步，畫成桌面 app。
...
- **寫之前一定先備份**：同目錄複製一份 `CLAUDE.md.bak-<西元8碼>-<時分秒>`，備份路徑會顯示在畫面上
...
只讀 `<年>/<月>/` 底下、檔名是八碼數字的 `.md`，例如 `2026/08/20260817.md`（西元年月日）。
...
5. 前四條都對不上，但那一行**有標狀態** —— 讓它自己成為一支工作項目，標題與專案就用那一行的

第 5 條的 id 是算出來的（`auto-<16 碼十六進位>`），算法固定，所以同一件事跨天寫還是同一支：
有連結就用連結當 key（先去掉結尾斜線、`?` 之後與 `#` 之後），沒連結就用「專案＋標題」當 key。
沒標狀態的行不走這條，只會出現在當天的日誌裡。標 `完成` 的行會走——看板要畫得出所有狀態，
一次性工作也得是一支項目才進得了「完成」那一欄。
...
- **全部項目**：所有工作項目
- **某一天**：只看那天有推進的項目，前後箭頭換日
- **一段期間**：只看那段期間裡有推進的項目

選期間就用日曆：切到「一段期間」時日曆會自己打開，**點一天當起始日，再點一天當結束日**
（先點晚的再點早的也可以，會自己對調）。選好的兩端是紅色實心，中間那幾天淡底。
範圍會寫進網址（`board.html?from=20260805&to=20260812`），重新整理不會跳掉。

欄頭點下去會進到那個狀態的清單頁（`status.html`），目前的範圍跟著帶過去。

## 狀態可以自己加

內建八個（待辦／提案中／暫存／實作中／測試中／待合併／已歸檔／完成）寫死在 `model.rs`，
但看板最右邊那一欄可以再加。加的時候要填名稱、一句說明，還有**插在哪一欄後面**——
欄序就是存下來的順序。顏色由 app 自動配一個沒用過的。

加出來的狀態跟內建的一樣：卡片拖得進去、寫進日誌檔的就是它的中文標籤。整張表存在
設定目錄的 `statuses.json`，開機時灌進 `model.rs` 的狀態表，解析與寫檔兩邊共用。

加完會問要不要把它寫進 `~/.claude/CLAUDE.md` 的日誌規則——**不寫進去的話，看板認得它，
但 Claude 寫日誌時不會用它**。按了才寫，寫之前一樣先備份。寫進去的時候會動兩個地方：
生命週期那行（插在指定的狀態後面）、狀態表格（多一列）。

自訂狀態可以刪（欄頭滑過去右上角有叉），內建八個刪不得。刪掉不會動日誌檔一個字——
已經寫成那個標籤的行留在原地，只是 app 之後不認得它。
...
| 看板 | 八欄（狀態可自己加），卡片可拖曳改狀態（直接寫進今天的 md）；範圍可切全部／某一天／一段期間；欄頭點得進該狀態的清單頁 |
...
- **全部**：讀到的每一天
- 每段右邊有「只看這天」可以切回單日
- 目前範圍會寫進網址（`daily-log.html?d=20260817&range=week`），重新整理不會跳掉
...
| 加到今日日誌 | 真的寫進今天的 `<年>/<月>/<西元8碼>.md`，寫完那筆標成完成（不刪掉） |
...
看板上把卡片拖到別欄（或用卡片右上角的「移動到…」），app **當下就改今天的 `<年>/<月>/<西元8碼>.md`**，
...
寫完會浮一則提示，例如「已寫進 20260820.md：測試中」，資料也立刻重讀，日誌頁與月曆同步跟上。
